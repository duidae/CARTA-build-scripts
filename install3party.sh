...
## for building qwt by qt for carta
if [ -z ${QT5PATH+x} ]; then
	echo "QT5PATH is unset";
	QT5PATH=/
	if [ "$(uname)" == "Darwin" ]; then
		QT5PATH=$HOME/Qt/5.7/clang_64
	else
		QT5PATH=/opt/Qt/5.7/gcc_64
	fi
...
if [ "$(uname)" == "Darwin" ] && [ "$TRAVIS" == true ]; then
	echo "Install protocol buffer in Mac is under construction..."
else
	echo "Install protocol buffer 3.6.0"
...
# Install micro websocket(uWebSocket)
if [ "$(uname)" == "Darwin" ] && [ "$TRAVIS" == true ]; then
	echo "Install uWebSocket in Mac is under construction..."
else
	echo "Install uWebSocket"
